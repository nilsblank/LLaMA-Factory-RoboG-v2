...
class BaseModelOutputWithDeepstackFeatures(ModelOutput):
    r"""
    Base class for vision model's outputs that also contains deepstack features.
    ...
    def get_video_features(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        def _compress_one_video(
            ...
            D = tokens_1d.shape[-1]
            tokens_tf = tokens_1d.view(T, per_frame, D)

            frame_qs = []
            for t in range(T):
                x = tokens_tf[t].unsqueeze(0)  # [1, per_frame, D]

                # Language conditioning (TimeChat-style): inject pooled timestamp embedding into the visual tokens.
                if self.timechat_qformer_text_input and ts_ids is not None:
                    ids_t = ts_ids[t] if ts_ids.dim() == 2 else ts_ids
                    mask_t = None
                    if ts_mask is not None:
                        mask_t = ts_mask[t] if ts_mask.dim() == 2 else ts_mask

                    emb = self.language_model.embed_tokens(ids_t.to(x.device))  # [L, text_dim]
                    if emb.shape[-1] == D:
                        if mask_t is None:
                            pooled = emb.mean(dim=0, keepdim=True)  # [1, D]
                        else:
                            m = mask_t.to(emb.device).float()
                            denom = m.sum().clamp(min=1.0)
                            pooled = (emb * m.unsqueeze(-1)).sum(dim=0, keepdim=True) / denom
                        x = x + pooled.view(1, 1, D)

                q = self.timechat_frame_resampler(x)[0]  # [1, Qf, D]

                # Frame position embedding (learned) for temporal order.
                pos = torch.tensor([min(t, self.timechat_max_frame_pos - 1)], device=q.device, dtype=torch.long)
                q = q + self.timechat_frame_pos_emb(pos).unsqueeze(1)
                frame_qs.append(q)

            q_time = torch.cat(frame_qs, dim=1)  # [1, T*Qf, D]
            ...
            for st in range(0, T, stride):
                ed = min(st + win, T)
                clip = q_time[:, st * self.timechat_num_frame_queries : ed * self.timechat_num_frame_queries, :]
        ...
        # Compress deepstack features (if present) so they stay aligned with the new visual token stream.
