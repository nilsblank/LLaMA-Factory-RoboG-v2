...
def vllm_infer(
    ...
    model_name_or_path = cfg.get("output_dir")
    adapter_name_or_path = cfg.get("adapter_name_or_path", None)
    dataset = cfg.get("eval_dataset", "alpaca_en_demo")
    dataset_dir = cfg.get("dataset_dir", "data")
    template = cfg.get("template", "default")
    cutoff_len = cfg.get("cutoff_len", 2048)
    max_samples = cfg.get("max_samples", None)
    vllm_config = cfg.get("vllm_config", "{}")
    save_name = cfg.get("save_name", "generated_predictions.jsonl")
    temperature = cfg.get("temperature", 0.95)
    top_p = cfg.get("top_p", 0.7)
    top_k = cfg.get("top_k", 50)
    max_new_tokens = cfg.get("max_new_tokens", 1024)
    repetition_penalty = cfg.get("repetition_penalty", 1.0)
    skip_special_tokens = cfg.get("skip_special_tokens", True)
    default_system = cfg.get("default_system", None)
    enable_thinking = cfg.get("enable_thinking", True)
    seed = cfg.get("seed", None)
    pipeline_parallel_size = cfg.get("pipeline_parallel_size", 1)
    image_max_pixels = cfg.get("image_max_pixels", 768 * 768)
    image_min_pixels = cfg.get("image_min_pixels", 32 * 32)
    video_fps = cfg.get("video_fps", 2.0)
    video_maxlen = cfg.get("video_maxlen", 128)
    # Note: batch_size is named differently in the config
    batch_size = 1024
    ...
    for i in tqdm(range(0, len(train_dataset), batch_size), desc="Processing batched inference"):
        ...
        images = [[] for _ in range(len(batch["input_ids"]))]
        videos = [[] for _ in range(len(batch["input_ids"]))]
    ...
    save_name = os.path.join(model_name_or_path, "generated_predictions.jsonl")
    
    # Write all results at once outside the loop
    with open(save_name, "w", encoding="utf-8") as f:
        for text, pred, label, image, video in zip(all_prompts, all_preds, all_labels, all_images, all_videos):
            f.write(json.dumps({"prompt": text, "predict": pred, "label": label, "image": image, "video": video}, ensure_ascii=False) + "\n")
